var defaultExpenses = [
  ...
  { name: "Cost Code 5", value: 0 },
  { name: "Cost Code 6", value: 0 },
];
...
var calculateOutput = function () {
  ...
  budget.materialCosts = overheadSum
...
var setupDefault = function () {
  ...
  expenses = localStorage.getItem("saved_materials1") ? JSON.parse(localStorage.getItem("saved_materials1")) : expenses
  console.log('m', expenses)

  for (let index = 0; index <= expenses.length - 2; index++) {
    console.log(index)
    $(".add_list_item").eq(0).click();
  }
...
var setupButtonEvents = function () {
  jQuery(".clear-values").click(function () {
    localStorage.setItem("saved_materials1", JSON.stringify(defaultExpenses));
    location.reload();
  });
...
saveData = function () {
  ...
  localStorage.setItem("saved_materials1", JSON.stringify(expenses));
  }
};
...
$(document).ready(function () {
  ...
  if (/[?&]onboarding=true/.test(location.search)) {
    ...
    $("#gform_submit_button_56").val('Next');
  } else if (/[?&]return=budget/.test(location.search)) {
    console.log('return budget')
    $(".email-field-block .gfield_label").hide();
    $("#gform_submit_button_56").val('Finish and return to Budgeting Calculator');
  }
