...
jQuery(document).on(
  ...
  function (event, form_id, current_page) {
    ...
    if (current_page == 1 && location.host != "localhost:8000") {
      ...
      console.log('gform_next_button click')
    }
...
var setupDefault = function () {
  ...
  expenses = localStorage.getItem("saved_sales1") ? JSON.parse(localStorage.getItem("saved_sales1")) : expenses
...
var setupButtonEvents = function () {
  jQuery(".clear-values").click(function () {
    localStorage.setItem("saved_sales1", JSON.stringify(defaultExpenses));
    location.reload();
  });
...
saveData = function () {
  ...
  if (expenses && expenses.length > 0) {
    localStorage.setItem("saved_sales1", JSON.stringify(expenses));
  }
};
...
$(document).ready(function () {
  ...
  if (/[?&]onboarding=true/.test(location.search)) {
    ...
    $("#gform_submit_button_57").val('Next');
